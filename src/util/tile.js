...
export function getInitialTileList() {
  ...
  tileList.push(tile2); // 처음에 2개 생성되니까 2번 호출
  return tileList;
}

export function checkCollision(tileList, tile) {
  return tileList.some((item) => item.x === tile.x && item.y === tile.y); // 만족하면 true 반환
}
...
export function makeTile(tileList) {
  ...
  while (!tile || (tileList && checkCollision(tileList, tile))) {
    // tile이 없거나 방향이 겹치는 경우 새로 타일을 만듬!!!
    tile = {
      id: currentId++,
...
export function moveTile({ tileList, x, y }) {
  assert(x === 0 || y === 0, ""); // prod 빌드할 때 제거하면 좋다
  const isMoveY = y !== 0;
  const isMinus = x + y < 0;
